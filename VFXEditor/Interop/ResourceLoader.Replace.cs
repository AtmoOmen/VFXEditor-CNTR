...
#nullable disable
        ...
        private void* GetResourceHandler(
            ...
            if( replacedPath == null || replacedPath.Length >= 260 ) {
                ...
                if( Plugin.Configuration?.LogDebug == true && DoDebug( gamePathString ) ) PluginLog.Log( "[GetResourceHandler] 原函数 {0} -> {1} -> {2}", gamePathString, replacedPath, new IntPtr( unreplaced ).ToString( "X8" ) );
                return unreplaced;
            }
            ...
            *resourceHash = InteropUtils.ComputeHash( resolvedPath.InternalName, resParams );
            ...
            if( Plugin.Configuration?.LogDebug == true ) PluginLog.Log( "[GetResourceHandler] 替换 {0} -> {1} -> {2}", gamePathString, replacedPath, new IntPtr( replaced ).ToString( "X8" ) );
            return replaced;
        }
        ...
        private byte ReadSqpackHandler( IntPtr pFileHandler, SeFileDescriptor* pFileDesc, int priority, bool isSync ) {
            ...
            if( gameFsPath == null || gameFsPath.Length >= 260 || !isRooted || isPenumbra ) {
                if( Plugin.Configuration?.LogDebug == true ) PluginLog.Log( "[ReadSqpackHandler] 使用 {0} 调用原函数", originalPath );
                return ReadSqpackHook.Original( pFileHandler, pFileDesc, priority, isSync );
            }

            if( Plugin.Configuration?.LogDebug == true ) PluginLog.Log( "[ReadSqpackHandler] 已使用 {0} 替换", gameFsPath );
